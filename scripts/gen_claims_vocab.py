...
GOLD_DIR = ROOT / "data" / "gold"          # held-out gold standards (experiment 11)
README = CLAIMS_DIR / "README.md"

# NOTE: this vocabulary is documentation for human annotators. It is deliberately
# NOT wired into src/agents/extractor.py's PROPERTY_VOCAB — the extraction prompt
# stays frozen so held-out results remain comparable, and so a held-out
# document's vocabulary never leaks into the prompt that extracts it.
...
MEANINGS = {
    ...
    # --- added for the held-out Samsung INR18650-25R gold (experiment 11) ---
    # No canonical equivalent existed for these; the datasheet states the charge
    # durations in minutes, so they cannot reuse `std_charge_time_h` without
    # rewriting the stated value.
    ...
    # Relative-capacity table cells: each is one cell of a datasheet grid, given
    # as a percentage of a reference capacity stated in the claim's conditions.
...
def _gold_files() -> list[Path]:
    """Shipped gold-standard YAMLs: the claims corpus plus the held-out golds.
    Annotation TEMPLATEs carry placeholder claims and are excluded."""
    files = list(CLAIMS_DIR.glob("*.yaml"))
    files += [p for p in GOLD_DIR.glob("*.yaml") if "TEMPLATE" not in p.name]
    return sorted(files)
